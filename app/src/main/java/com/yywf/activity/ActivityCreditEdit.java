...
public class ActivityCreditEdit extends BaseActivity implements OnClickListener {

    private final String TAG = "ActivityCreditEdit";
    ...
    private void doGetApproveInfo() {
        ...
        params.add("cardId", id+"");
        HttpUtil.get(ConfigXy.CREDIT_QUERY, params, requestListener1);
    }
        ...
        public void success(String response) {
            ...
            try {
                ...
                if (!result.optBoolean("status")) {
                    ...
                    String id_no = dataStr.optString("idNo");
                    String member_name = dataStr.optString("idName");
                    cardNo = dataStr.optString("cardNum");
                    String phone = dataStr.optString("phone");
                    String cardCvn = dataStr.optString("cardCvn");
                    String billDate = dataStr.optString("billDate");
                    String payDate = dataStr.optString("payDate");
                    String cardExpdate = dataStr.optString("cardExpdate");
                    ...
                    if (!StringUtils.isBlank(cardNo)){
                        et_card_no.setEnabled(false);
                        et_card_no.setText(StringUtils.formatCardNo(cardNo));
                    }
                    if (!StringUtils.isBlank(phone)){
                        et_phone.setText(phone);
                    }
                    if (!StringUtils.isBlank(cardCvn)){
                        et_cvv2.setText(cardCvn);
                    }
                    if (!StringUtils.isBlank(billDate)){
                        et_bill_day.setText(billDate);
                    }
                    if (!StringUtils.isBlank(payDate)){
                        et_repayment_day.setText(payDate);
                    }
                    if (!StringUtils.isBlank(cardExpdate)){
                        et_date.setText(cardExpdate);
                    }
                }
...
//        String bandCard = StringUtils.replaceBlank(et_card_no.getText().toString().trim());
        ...
        if (!StringUtils.checkBankCard(cardNo)) {
...
//        }
        ...
        params.add("cardId", id);
        HttpUtil.get(ConfigXy.XY_CREDIT_EDIT, params, requestListener);
