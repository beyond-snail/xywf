...
public class ImageIndicatorView extends RelativeLayout {
	...
	public void show() {
		...
		for (int index = 0; index < this.totelCount; index++) {
			...
			this.indicateLayout.addView(indicater, index, lp);
		}
		...
		for (int index = 0; index < totelCount; index++) {
			...
			if (isShowIndicator) {
				if (this.currentIndex == index) {
					imageView.setBackgroundResource(R.drawable.home_lunbo_circle_pre);
				} else {
					imageView.setBackgroundResource(R.drawable.home_lunbo_circle);
				}
			}
		}
