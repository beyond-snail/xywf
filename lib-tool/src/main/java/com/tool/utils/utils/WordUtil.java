...
public class WordUtil {
	...
	public WordUtil(String doc_name) {
		docPath = doc_name;
		htmlPath = FileWordUtil.createFile("html", FileWordUtil.getFileName(docPath) + ".html");
			...
			while (event_type != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.START_TAG: // 开始标签
					...
					if (tagBegin.equalsIgnoreCase("pic")) { // 检测到图片
						ZipEntry pic_entry = FileWordUtil.getPicEntry(docxFile, pic_ndex);
						if (pic_entry != null) {
							byte[] pictureBytes = FileWordUtil.getPictureBytes(docxFile, pic_entry);
...
//						}
	...
	public void writeDocumentPicture(byte[] pictureBytes) {
		picturePath = FileWordUtil.createFile("html", FileWordUtil.getFileName(docPath) + presentPicture + ".jpg");
		FileWordUtil.writePicture(picturePath, pictureBytes);
		presentPicture++;
